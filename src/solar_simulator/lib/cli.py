"""Interactive menu for the Solar Simulator.

Only the `complete` build ships this module.
"""
...
class Cli:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if mode == "1":
                AutoMode(self.sim).run()
                break

            if mode == "2":
                ManualMode(self.sim).run()
                break
    ...
    def _input_with_default(
        prompt: str, default_value: str, valid_values: tuple = (), value_type: type = str
    ) -> None:
