...
class SolarSimulator:
    def __init__(self, pwm_freq: int = 5000, verbose: int = 0):
        """Initialize a SolarSimulator object."""
        ...
        if self.verbose >= 2: print(f"I2C initialized, addresses found: {self._portScan()}")
    ...
    def setLEDs(self, v: int = 0, w: int = 0, c: int = 0, h: int = 0):
        """Set the LEDs and halogen brightness as a 16-bit integer value."""
    ...
    def checkThermals(self) -> list:
        ...
        thermals = []
        thermistors = self._readThermistors()
    ...
    def _readThermistors(self) -> list:
